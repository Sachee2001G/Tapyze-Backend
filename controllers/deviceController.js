  ...
  try {
    ...
    if (req.params.customerId && req.user.type !== "Admin") {
      return res.status(403).json({
        status: "error",
        message:
          "You do not have permission to assign cards to other customers",
      });
    }
...
export const changeCardPin = async (req, res, next) => {
  try {
    ...
    // Verify ownership
    if (
      card.owner.toString() !== userId.toString() &&
      req.user.type !== "Admin"
    ) {
      return res.status(403).json({
        status: "error",
...
export const getCustomerCards = async (req, res, next) => {
  try {
    ...
    if (
      ...
      req.user.type !== "Admin" &&
      req.user.id.toString() !== req.params.customerId
    ) {
      return res.status(403).json({
        status: "error",
  ...
  try {
    ...
    if (req.params.merchantId && req.user.type !== "Admin") {
      return res.status(403).json({
        status: "error",
        message:
          "You do not have permission to assign scanners to other merchants",
      });
    }
...
export const getMerchantScanners = async (req, res, next) => {
  try {
    ...
    if (
      ...
      req.user.type !== "Admin" &&
      req.user.id.toString() !== req.params.merchantId
    ) {
      return res.status(403).json({
        status: "error",
